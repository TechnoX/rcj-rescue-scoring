...
angular.module("LineScore", ['datatables', 'ui.bootstrap', 'ngAnimate']).controller("LineScoreController", function ($scope, $http, $sce) {
  ...
  function sum_jpop(runs) {
    if (runs.length == 1) {
      return {
        score  : runs[0].score,
        time   : runs[0].time,
        rescued: runs[0].rescuedLiveVictims + runs[0].rescuedDeadVictimsBefore + runs[0].rescuedDeadVictimsAfter,
        lops   : runs[0].LoPsNum,
    ...
    for (var i = 0; i < not_select.length; i++) {
      ...
      result.rescued += not_select[i].rescuedLiveVictims +
                        not_select[i].rescuedDeadVictimsBefore +
    ...
    result.rescued += select[0].rescuedLiveVictims +
                      select[0].rescuedDeadVictimsBefore +
                      select[0].rescuedDeadVictimsAfter;
  ...
  function sumBest(runs) {
    ...
    for (let i = 0; i < Math.min(2, runs.length); i++) {
      ...
      sum.time.seconds %= 60
      sum.rescued += runs[i].rescuedLiveVictims + runs[i].rescuedDeadVictimsBefore + runs[i].rescuedDeadVictimsAfter
